...
void TMVP_recursive(type64 *F, type64 *G, type64 *Z){
	...
	T6[0]=F[3];				T6[1]=F[4];				T6[2]=F[2];
	T6[3]=F[5];				T6[4]=F[1];		
	tmvp(T6, T1, M2);

	T1[0]=G[3]-G[6];		T1[1]=G[4]-G[7];		T1[2]=G[5]-G[8];
	T6[0]=T1[3];			T6[1]=c;				T6[2]=T1[4];
	T6[3]=T5;				T6[4]=2*F[4];
	tmvp(T6, T1, M4);

	T1[0]=G[0]-G[6];		T1[1]=G[1]-G[7];		T1[2]=G[2]-G[8];
	T6[0]=F[0];				T6[1]=F[1];				T6[2]=T5;
	T6[3]=F[2];				T6[4]=c;
	tmvp(T6, T1, M3);

	T6[0]=F[6]+F[3];		T6[1]=F[7]+F[4];		T6[2]=F[5]+F[2];
	T6[3]=F[8]+F[5];		T6[4]=F[4]+F[1];	
	T1[0]=T6[0]+F[0];		T1[1]=T6[1]+F[1];		T1[2]=T6[2]+T5;
	T1[3]=T6[3]+F[2];		T1[4]=T6[4]+c;
