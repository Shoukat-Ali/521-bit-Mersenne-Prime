...
void TMV_multiplication(type64 *X, type64 *Y, type64 *Z){
	...
	C = (((type128)T6[0]*Y[8])+((type128)T6[1]*Y[7])+((type128)T6[2]*Y[6]) + X13 + X16) + 2*(C>>58);
...
w[130]= 1;
	...
	cout<<"Window width :: "<<WINDOW<<endl<<"The minimum mean number of clock cycles is :: "<<min_ccycle<<endl;
	//output(&P);
