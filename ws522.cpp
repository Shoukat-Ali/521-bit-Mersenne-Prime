...
// M.Scott 27/02/2015
// We have just replaced gmul() by TMV_multiplication() because this version is for modulus 2p
...
// We have made some changes in this function according to our algorithm need and it's commented
...
where FG = Z (mod pow(2,522) - 2) and both F and G comprised of 9-limb regardless of their bitlength
F[i] and G[i] are unsigned 58-bit where i=1,2,3,4,5,6,7,8
F[0], G[0], and Z[0] are at most signed 60-bit */
...
void TMV_multiplication(type64 *F, type64 *G, type64 *Z){
	...
	c = ((type64)C)&bot58bits;
	C = (((type128)T6[0]*G[8])+((type128)T6[1]*G[7])+((type128)T6[2]*G[6]) + X13 + X16) + 2*(C>>58);
...
w[130]= 1;
	...
	cout<<"Window width :: "<<WINDOW<<endl<<"The minimum number of clock cycles is :: "<<min_ccycle<<endl;
	//output(&P);
